...
const ShelfWithBooks = ({ myBooks, setMyBooks }) => {
    const svgRef = useRef();
    const [books, setBooks] = useState([]); // 棚の本
    ...
    useEffect(() => {
                ...
                .call(
                    d3.drag()
                        ...
                        .on("end", function (event, d) {
                            ...
                            let targetShelf = shelves[d.shelfIndex];
        ...
        // SVG 上で手元本のドロップを受け付ける
        const container = svgContainerRef.current;
        container.ondragover = (e) => e.preventDefault();
        container.ondrop = (e) => {
            e.preventDefault();
            const bookId = e.dataTransfer.getData("text");
            const newBook = myBooks.find(b => b.id === bookId);
            if (!newBook || books.some(b => b.id === newBook.id)) return;

            // 左端の棚に追加
            const shelf = shelves[0];
            const addedBook = { ...newBook, shelfIndex: 0 };
            shelf.books.push(addedBook);
